...
class SequenceRunner(threading.Thread):
    ...
    def _maybe_sim_click(self) -> bool:
        ...
        if success:
            ...
            if self.plan.get("pixel_detection_enabled"):
                delay = max(0.0, float(self.plan.get("pixel_post_click_delay", 0.15)))
                if delay:
                    time.sleep(delay)
                self._refresh_pixel_baseline()
        return success

    def _handle_post_stable_actions(self):
        click_success = self._maybe_sim_click()
        if not self.plan.get("pixel_detection_enabled"):
            return
        region = self.plan.get("pixel_detection_region")
        if not region or len(region) != 4:
            ...
            return
        if not click_success and self.pixel_baseline is None:
            # 没有点击也需要刷新一次基准
            delay = max(0.0, float(self.plan.get("pixel_post_click_delay", 0.15)))
            if delay:
                time.sleep(delay)
        ...
        if self.pixel_baseline is None and not self._refresh_pixel_baseline():
            return
    ...
    def _record_result(self, temp, pressure, temp_status, press_status):
        self._handle_post_stable_actions()
